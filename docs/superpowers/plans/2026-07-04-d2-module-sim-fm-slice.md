...
- **`native_sim` never in prod OR standard dev builds.** It (and the harness) live only in the new `oe5xrx-remotestation-sim-image` variant, via `packagegroup-oe5xrx-sim`. The **real firmware `.bin`** (DFU source) and the **udev ruleset** (harmless without hardware) DO go into the base/prod image.
