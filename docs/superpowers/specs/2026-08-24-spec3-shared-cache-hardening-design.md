...
- **Remove the hard RPi freeze** — delete `meta-oe5xrx-remotestation/dynamic-layers/raspberrypi/recipes-kernel/linux/linux-raspberrypi_6.18.bbappend` (its only content is the `LINUX_VERSION = "6.18.39"` + `SRCREV_machine` pin). **Not load-bearing.** The bbappend's comment claims a kernel-level CM4 USB fix (the FM module's DTR `SET_CONTROL_LINE_STATE` control transfer timing out on meta-raspberrypi's 6.18.33 default), but that hypothesis was wrong: the real cause was in `station-manager` — a plain `open()` on the serial port that corrupted the port state — **fixed by switching to pyserial**. So the kernel version is irrelevant to the FM module and the pin chases a phantom. (The stale comment goes away with the file.) A hard `SRCREV` never served qemu↔RPi parity anyway (the two kernels are different forks with unrelated SRCREVs; see below). `BB_DANGLINGAPPENDS_WARNONLY` is not set anywhere, and removing the bbappend removes the dangling-append situation the old comment referenced.
